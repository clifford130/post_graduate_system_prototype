...
document.addEventListener("DOMContentLoaded", async () => {
  let response = await fetch("https://post-graduate-system-prototype-xy2c.onrender.com/api/islogged", {
    method: "POST",
    credentials: "include"
  })
...
export async function initShell() {
  ...
  try {
    ...
    if (u && u.id && !u.fullName) {
      const res = await fetch(`https://post-graduate-system-prototype-xy2c.onrender.com/api/students/${encodeURIComponent(u.id)}`);
      if (res.ok) {
